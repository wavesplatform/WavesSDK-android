...
//			Debug.dumpArray("h @ compress end", h);
    ...
    public static class Param implements AlgorithmParameterSpec {
        ...
        public final Param setDigestLength(int len) {
            ...
            assert len <= Spec.max_digest_bytes : Engine.Assert.assertFail("len", len, Engine.Assert.inclusiveUpperBound, Spec.max_digest_bytes);
        ...
        public final Param setKey (final byte[] key) {
            assert key != null : "key is null";
            assert key.length >= 0 : Engine.Assert.assertFail(
                    "key.length", key.length, Engine.Assert.inclusiveUpperBound, 0);
            assert key.length <= Spec.max_key_bytes : Engine.Assert.assertFail(
                    "key.length", key.length, Engine.Assert.inclusiveUpperBound, Spec.max_key_bytes);
        ...
        public final Param setFanout(int fanout) {
            assert fanout > 0 : Engine.Assert.assertFail(
        ...
        public final Param setDepth(int depth) {
            assert depth > 0 : Engine.Assert.assertFail(
        ...
        public final Param setLeafLength(int leaf_length) {
            assert leaf_length >= 0 : Engine.Assert.assertFail(
        ...
        public final Param setNodeOffset(long node_offset) {
            assert node_offset >= 0 : Engine.Assert.assertFail(
        ...
        public final Param setNodeDepth(int node_depth) {
            assert node_depth >= 0 : Engine.Assert.assertFail(
        ...
        public final Param setInnerLength(int inner_length) {
            assert inner_length >= 0 : Engine.Assert.assertFail(
        ...
        public final Param setSalt(final byte[] salt) {
            ...
            assert salt.length <= Spec.max_salt_bytes : Engine.Assert.assertFail(
                    "salt.length", salt.length, Engine.Assert.inclusiveUpperBound, Spec.max_salt_bytes);
        ...
        public final Param setPersonal(byte[] personal) {
            ...
            assert personal.length <= Spec.max_personalization_bytes : Engine.Assert.assertFail("personal.length", personal.length, Engine.Assert.inclusiveUpperBound, Spec.max_personalization_bytes);
