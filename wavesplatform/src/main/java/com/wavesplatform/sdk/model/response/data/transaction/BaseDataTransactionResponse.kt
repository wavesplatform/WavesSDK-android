...
abstract class BaseDataTransactionResponse(
        /**
        ...
        var height: Long? = null) : BaseTransactionParcelable {
    ...
    override fun readBaseFromParcel(parcel: Parcel) {
        ...
        senderPublicKey = parcel.readString().orEmpty()
        timestamp = parcel.readString().orEmpty()
        fee = parcel.readDouble()
        // chainId = parcel.readByte()
        version = parcel.readByte()
        parcel.readStringList(proofs)
        signature = parcel.readString().orEmpty()
